...
void UART5_IRQHandler(void)
{
			...
			static u16 ui = 0;
			static u16 StReceive = 0;
			if(USART_GetITStatus(PAD_USART, USART_IT_RXNE) != RESET)	   
			{		
					temp = USART_ReceiveData(PAD_USART);
		 ...
		 if(USART_GetITStatus(PAD_USART, USART_IT_TXE) != RESET) 
			{ 
					USART_ITConfig(PAD_USART, USART_IT_TXE, DISABLE);
			}
	#else
